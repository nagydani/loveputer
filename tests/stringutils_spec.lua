...
describe("StringUtils", function()
  ...
  -- describe('substrings UTF-8 text', function()
  --   it('splits', function()
  ...
  describe('substrings UTF-8 text with the other impl', function()
    it('splits', function()
      local s = StringUtils.utf8_sub_full(test1, 1, 3)
      assert.equal('ког', s)
    end)

    it('splits the first char', function()
      local s = StringUtils.utf8_sub_full(test1, 1, 1)
      assert.equal('к', s)
    end)

    it('trivially splits', function()
      local s = StringUtils.utf8_sub_full(test1, 1, 5)
      assert.equal(test1, s)
    end)
    ...
    it('handles overindexing', function()
      local s = StringUtils.utf8_sub_full(test1, 1, 6)
      assert.equal(test1, s)
    end)

    describe('indexes negatively', function()
      it('one off', function()
        local s = StringUtils.utf8_sub_full(test1, 1, -2)
        assert.equal('когд', s)
      end)
      ...
      it('suffix 1', function()
        local s = StringUtils.utf8_sub_full(test1, -1)
        assert.equal('а', s)
      end)

      it('suffix 2', function()
        local s = StringUtils.utf8_sub_full(test1, -2)
        assert.equal('да', s)
      end)

      it('suffix 2 ja', function()
        local s = StringUtils.utf8_sub_full(test2, -2)
        assert.equal('えお', s)
      end)
    end)
  end)
